...
import CallClientLink from './CallClientLink';
...
export default async function BusinessDetailPage({
  ...
  if (user) {
    const { data: ownerRecord } = await supabase
      ...
      .eq('id', user.id)
      .single();
    isOwner = ownerRecord?.business_id === biz.id;
  }
      ...
      {isOwner && (
        <div className="container" style={{ paddingTop: 'var(--space-6)' }}>
          <OwnerDashboard
            ...
            subscriptionTier={biz.subscriptionTier || 'free'}
            pageViews={biz.pageViews || 0}
            whatsappClicks={biz.whatsappClicks || 0}
            callClicks={biz.callClicks || 0}
          />
        </div>
      )}
